...
The `home-routes` handler in the `portfolio.routes.home` namespace
...
The `home-page` function will in turn call the `portfolio.layout/render` function
...
The routes are aggregated and wrapped with middleware in the `portfolio.handler` namespace:
